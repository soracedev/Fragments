...
function talkLui() {
  ...
  if (!S.flags.luiSpoke1) {
    ...
    speak(
      [
        LU(
          "Erano qui. Ne sono certo. Li avevo messi al sicuro… e adesso non ricordo più dove.",
        ),
  ...
  if (!has("chiavi-auto")) {
    if (!S.flags.fragCamera) {
      speak([
        SG(
          "LUI ha di nuovo lo sguardo perso. Quei «pezzi» che cerca devono essere da qualche parte, qui intorno.",
        ),
      ]);
      return;
    }
  ...
  if (!has("usb")) {
    if (!S.flags.fragGiardino) {
      speak([
        SG("LUI tace. Manca ancora qualcosa — un altro di quei frammenti."),
      ]);
      return;
    }
    speak(
      [
        LU(
          "Mi dispiace. Custodivo quei frammenti gelosamente, sperando di trovare il modo giusto per ripararli.",
        ),
        LU(
  ...
  speak([
    SG("LUI fissa il vuoto, in silenzio. Ha detto tutto quello che poteva."),
  ]);
}
...
const LEI_BASE = [
  ...
  P(
    "Non dire così… Ci sarà qualcosa che potrò fare per aiutarti. Come con l'orologio.",
  ),
...
const LEI_LAV_INTRO = [
  ...
  L(
    "Non riesco mai a farne una giusta. Sono una buona a nulla che ha solo buttato il suo tempo nel niente.",
  ),
  P(
...
const LEI_FINALE_2 = [
  ...
  P(
    "Perché tu sei quella parte di me che ha smesso di crederci. Ma non sei un errore da correggere.",
  ),
  L(
    "…quindi sono solo questo? Un promemoria di tutto ciò che c’è di sbagliato?",
  ),
  P(
    "No. Ti sei fatta carico di tutta la tristezza, delle ferite del passato. Hai accumulato le mie paure, le mie insicurezze — e io, nel frattempo, mi sono lasciata andare.",
  ),
  L("Quindi servo solo a questo. Sono un ostacolo. Qualcosa da eliminare."),
  P(
    "No. Sei la motivazione che mi spinge ad andare avanti, a migliorare. Mi ricordi la parte più negativa di me — ma non devo vederti come un male. Sei un’impronta di ciò che sono.",
  ),
  P(
    "Devo accettare la tua presenza. Capire che non si nasce già capaci di qualcosa — lo si diventa. Ed è anche grazie a te, se ci sto arrivando.",
  ),
  SG("Un silenzio lungo."),
  L("Anche se lo accettassi… non sparirò. Resterò comunque qui."),
  P("Lo so. E va bene così."),
  P(
    "Non voglio farti sparire. Voglio solo che tu smetta di parlare più forte di tutto il resto.",
  ),
  L("E se un giorno tornassi a urlare come prima?"),
  P(
    "Allora ti ascolterò di nuovo. E ti risponderò di nuovo, come sto facendo ora.",
  ),
  SG("La sua voce trema, per la prima volta senza difendersi."),
  L("Non so se sono pronta a crederci davvero."),
  P(
    "Non serve crederci subito. Basta provarci. Un po’ alla volta — come con l’orologio.",
  ),
];

// La battuta dell'abbraccio è separata: si gioca sopra l'illustrazione
// dedicata (scena "abbraccio"), non nell'interno lavanderia.
const ABBRACCIO_SG = SG(
...
const ACTIONS = {
  ...
  "auto-take": () => {
    if (has("chiavi-lavanderia")) {
      speak([
        P("Ho già dato un'occhiata. Dentro, solo le chiavi della lavanderia."),
      ]);
      return;
    }
    if (has("chiavi-auto")) {
      speak(
        [
          P(
            "Chissà perché le chiavi della lavanderia erano proprio qui dentro. Vale la pena dare un’occhiata.",
          ),
        ],
        () => {
          acquire(
            "chiavi-lavanderia",
            "Prendi le chiavi della lavanderia.",
            () => writeSave(),
          );
        },
      );
  ...
  "cancello-enter": () => {
    ...
    speak(
      [
        P(
          "L'auto di nonno non c'è, provo ad entrare lo stesso — magari è uscito solo lui ed è rimasta nonna.",
        ),
      ],
      () => fadeWhite(() => enterCasaf()),
    );
  },
  "closeup-cancello-back": () => {
    show("fuori-casa");
  },
  ...
  tavolino: () => {
    if (S.flags.tavoloDone) {
      speak([P("Lo stampo è pronto. Ora tocca al compressore.")]);
      return;
    }
    openTavolino();
  },
  ...
  compressore: () => {
    if (S.flags.compressoreDone) {
      speak([SG("Il set è completo. Non serve altro.")]);
      return;
    }
    if (!S.flags.tavoloDone) {
  ...
  "frammento-1": () => {
    ...
    acquire(
      "frammento-sinistro",
      "Un frammento di cuore, sul letto. Lo raccogli.",
      () => writeSave(),
    );
  },
  pc: () => {
    if (S.flags.usbLoaded) {
      speak([P("Ho già trasferito tutto sulla chiavetta.")]);
      return;
    }
    if (!(S.flags.fragCamera && S.flags.fragGiardino)) {
      speak([
        P(
          "Devo modellare qualcosa da stampare, ma mi servono prima dei pezzi da rimettere insieme.",
        ),
      ]);
      return;
    }
    if (!has("usb")) {
      speak([P("Mi servirebbe qualcosa su cui salvare — una chiavetta.")]);
      return;
    }
  ...
  stampante: () => {
    if (has("d6")) {
      speak([P("Il dado è già stampato. È qui con me.")]);
      return;
    }
    if (!S.flags.usbLoaded) {
      speak([P("Mi servirebbe prima qualcosa da poter stampare.")]);
      return;
    }
  ...
  "frammento-2": () => {
    ...
    acquire(
      "frammento-destro",
      "Un frammento di cuore, sul prato. Lo raccogli.",
      () => writeSave(),
    );
  },
  "giardino-back": () => {
