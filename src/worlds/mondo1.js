...
const TESTO_BIGLIETTINO_CASA =
  "Ero quasi tentata di provarci. Cosa mi costa? Non è un contratto a vita. " +
  "Poi mi sono ricordata che io non sono capace di fare quello che mi chiedono";

const TESTO_BIGLIETTINO =
  "Vorrei tanto provarci, capire se ne sono capace. " +
  "Eppure non riesco a muovermi da qui.";
...
export function refreshHotspots() {
  if (S.scene === "piazza") {
    ...
    if (hs("piazza-clock"))
      hs("piazza-clock").classList.toggle("done", S.flags.clockFixed);
  }
  ...
  if (S.scene === "casa-bagno") {
    setBagnoState();
    if (hs("specchio"))
      hs("specchio").classList.toggle("done", S.flags.mirrorSolved);
  }
...
export function arrivePiazza() {
  ...
  speak(
    [
      ...
      SG(
        "Un'ombra: una figura poco distinta, seduta su una panchina vicino all'orologio.",
      ),
      ...
      L(
        "Vedi quell'orologio? È fermo da... non lo so. Ho smesso di controllare, credo.",
      ),
      L(
      ...
      P(
        "Pensi che io sia in grado di poter uscire da qualunque sia questo posto e tornare a casa mia, se lo aggiusto?",
      ),
...
function talkFigure() {
  ...
  speak([
    L("Se vuoi riparare l’orologio, fa pure. A me non cambia nulla, credo."),
  ]);
}
...
function talkFigureBeach() {
  speak(
    [
      ...
      P(
        "E comunque questo smentisce il fatto che tu sia capace solo a restare immobile.",
      ),
      L("..."),
    ...
    () => {
      ...
      fadeWhite(() => {
        ...
        writeSave();
        refreshHotspots();
        if (typeof window.__refreshPrologueHotspots === "function")
          window.__refreshPrologueHotspots();
        speak([
          P("Sono riuscita a tornare a casa!"),
          P(
            "Stavo forse sognando? Chi era quella figura misteriosa? Forse stavo ancora dormendo?",
          ),
...
const ACTIONS = {
  ...
  "piazza-clock": () => {
    if (S.flags.clockFixed) {
      speak([P("Ticchetta di nuovo. Un rumore quasi dimenticato.")]);
      return;
    }
    if (!S.has.lancetta) {
      speak([
        P(
          "Manca la lancetta delle ore all'orologio. Forse riparandolo potrei uscire da questo posto.",
        ),
      ]);
      return;
    }
  ...
  "porta-casa": () => {
    if (!S.has.chiave) {
      speak([P("È chiusa. Ci vorrebbe una chiave.")]);
      return;
    }
    show("casa-soggiorno");
    refreshHotspots();
  ...
  quadro: () => {
    if (S.has.dado) {
      speak([P("La cassaforte è aperta. Non c’è rimasto altro, qui dietro.")]);
      return;
    }
    ...
    speak(
      [
        SG(
          "Un piccolo spessore fra il muro e il quadro. Sembra esserci qualcosa, qui dietro.",
        ),
      ],
      openSafe,
    );
  ...
  // --- CASA · BAGNO ---
  specchio: () => {
    if (S.flags.mirrorSolved) {
      speak([P("Il numero è ancora lì, disegnato nella condensa. 210525.")]);
      return;
    }
    openMirror();
  },
  ...
  saracinesca: () => {
    if (S.has.lancetta) {
      speak([P("Ho già preso quello che mi serviva, qui.")]);
      return;
    }
    if (!S.has.ingranaggio) {
      speak([
        P(
          "Una vecchia saracinesca. Sembra esserci qualcosa incastrato lì sotto, ovviamente nel punto più scomodo possibile.",
        ),
  ...
  ingranaggio: () => {
    if (S.has.ingranaggio) {
      speak([
        P(
          "La fontana è vuota. Solo alghe, e acqua che non va da nessuna parte.",
        ),
      ]);
      return;
    }
